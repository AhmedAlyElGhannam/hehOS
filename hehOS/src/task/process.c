...
static int process_map_elf(struct process* process)
{
    ...
    for (int i = 0; i < header->e_phnum; i++)
    {
        ...
        res = paging_map_to(process->task->page_directory, paging_align_to_lower_page((void*)phdr->p_vaddr), paging_align_to_lower_page(phdr_phys_address), paging_align_address(phdr_phys_address+phdr->p_memsz), flags);        if (ISERR(res))
        {
            break;
        }
    }
...
{
    int res = -ENOMEM;
    for (int i = 0; i < HEHOS_MAX_PROGRAM_ALLOCATIONS; i++)
    {
...
void process_free(struct process* process, void* ptr)
{
    ...
    if (!allocation)
    {
        return; // not the process's pointer
    }
