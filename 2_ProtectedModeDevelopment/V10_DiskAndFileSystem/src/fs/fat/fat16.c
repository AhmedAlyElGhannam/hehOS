...
#define HEHOS_FAT16_SIGNATURE       0x29
#define HEHOS_FAT16_FAT_ENTRY_SIZE  0x02
#define HEHOS_FAT16_BAD_SECTOR      0xFF7
#define HEHOS_FAT16_UNUSED          0x00
...
struct fat_h
{
    ...
    union fat_h_e // union cuz it is optional and to save memory
    {
...
struct fat_directory_item
{
    ...
    uint8_t attribute;
    uint8_t reserved; // reserved for future improvements
...
// easier way to deal with fat_directory_item struct
struct fat_directory
{
    struct fat_directory_item* item;
    int total; // total number of items in the fat directory
    int sector_pos; // first sector where this fat dir data is
    int ending_sector_pos; // last sector where this fat dir data is
};
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != HEHOS_CAPICHE)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != HEHOS_CAPICHE)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    {
        total_sectors += 1; // rounding total number of sectors to account for small items
    }
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != HEHOS_CAPICHE)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != HEHOS_CAPICHE)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    // read disk stream and if it is not ok, i/o error
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != HEHOS_CAPICHE)
    {
        res = -EIO;
        goto out;
    }

    // does the loaded header have a valid FAT16 signature?
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != HEHOS_CAPICHE)
    {
        res = -EIO;
        goto out;
    }
...
// fat16 specs state that file names MUST be filled and terminated with spaces
// this function converts fat16-formated names into normal strings
void fat16_to_proper_string(char** out, const char* in)
{
    // as long as the char in is not null or space, copy
    ...
    // terminate out with null if last char in 'in' was space 
    if (*in == 0x20)
    {
        **out = 0x00;
    }
...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len)
{
    ...
    // if this is true, file has extension
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * HEHOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        // check if sector if marked bad
        if (entry == HEHOS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[HEHOS_MAX_PATH];
    for (int i = 0; i < directory->total; i++)
    {
        // appends filename and extension
        fat16_get_full_relative_filename(&directory->item[i], tmp_filename, sizeof(tmp_filename));
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0)
        {
            // file found => create a new fat item
