...
{
    // compare file's signature with elf signature
...
{
    // only support little endian since target hardware is 32-bit x86
...
{
    // only allow elf to execute iff its entrypoint is a valid virtual address
    return header->e_type == ET_EXEC && header->e_entry >= HEHOS_PROGRAM_VIRTUAL_ADDRESS;
}

static bool elf_has_program_header(struct elf_header* header)
{
    // only load files with program headers
...
{
    // extracting section header from file header
...
// basically utilize all the tests created above to validate this elf file
int elf_validate_loaded(struct elf_header* header)
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? HEHOS_ALL_OK : -EINFORMAT;
}
...
// only supports running static elf files
// will not run elf that depend on shared libs or relocation files
// ALL symbols must be known before runtime
